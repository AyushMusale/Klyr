...
import 'package:klyr/presentation/expenses/personal/widgets/expense_edit_button.dart';
import 'package:klyr/presentation/expenses/personal/widgets/expense_field_label.dart';
import 'package:klyr/presentation/expenses/personal/widgets/expense_skeleton.dart';
...
class ShowPersonalExpPage extends StatefulWidget {
  final int id;
  const ShowPersonalExpPage({super.key, required this.id});

  @override
  State<ShowPersonalExpPage> createState() => _ShowPersonalExpPageState();
}

class _ShowPersonalExpPageState extends State<ShowPersonalExpPage> {
  late final TextEditingController _amountController;
  late final TextEditingController _titleController;
  late final TextEditingController _dateController;
  bool isEdditable = false;
  ...
  void initState() {
    super.initState();
    context.read<ExpenseBloc>().add(GetExpenseByIDEvent(id: widget.id));
  ...
  void _onDelete() {
    context.read<ExpenseBloc>().add(DeleteExpenseEvent(id: widget.id));
  }
              ...
              children: [
                    ...
                    child: BlocConsumer<ExpenseBloc, ExpenseState>(
                      listener: (context, state) {
                        ...
                        if (state is ExpenseLoadedState) {
                          _amountController.text = state.expense?.amount ?? '';
                          _titleController.text = state.expense?.title ?? '';
                          final date = state.expense?.date;
                          _dateController.text =
                              '${date?.day.toString().padLeft(2, '0')}-${date?.month.toString().padLeft(2, '0')}-${date?.year}';

                          _selectedCategoryIndex = expenseCategories.indexWhere(
                            (c) =>
                                c.apiValue ==
                                state.expense?.category.toUpperCase(),
                          );
                          setState(() {});
                        } else if (state is ExpenseUnauthState) {
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text(state.message)),
                          );
                      ...
                      builder: (context, state) {
                        if (state is ExpenseFailureState) {
                          ...
                          return const ShowPersonalExpSkeleton();
                        }
                        return Column(
                ...
                Padding(
                  ...
                  child: BlocBuilder<ExpenseBloc, ExpenseState>(
                    builder: (context, state) {
                      if (isEdditable) {
                        return state is ExpenseLoadingState
